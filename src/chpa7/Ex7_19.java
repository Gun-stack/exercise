...
class Buyer {
	...
	void buy(Product p) {
		if (money < p.price) {
			...
			System.out.println("금액이 부족합니다. 부족금액 :  " + (p.price - money));
			return;//메소드가 끝
		}
		money -= p.price;
		System.out.println(p + "를 구입합니다 가격 : "+p.price);
		
		add(p);
		/*
		 * (1) 아래의 로직에 맞게 코드를 작성하시오. 1.1 가진 돈과 물건의 가격을 비교해서 가진 돈이 적으면 메서드를 종료한다. 1.2
		 * 가진돈이 충분하면, 제품의 가격을 가진 돈에서 빼고 1.3 장바구니에 구입한 물건을 담는다.(add메서드 호출)
		 */
	}
	...
	void summary() {
		System.out.print("구입 목록 : ");
		for (int j = 0; j < i; j++) {
			System.out.print(cart[j].toString() + " " + cart[j].price+" ");
		}
		int tot = 0;
		for (int j = 0; j < i; j++) {
			tot += cart[j].price;
